                             ...
                             std::string const & dof_index_velocity_external_in)
{
  ...
  if(param.diffusive_problem())
  {
    ...
    diffusive_operator_data.bc =
      GeneralizedLaplace::Boundary::create_laplace_boundary_descriptor<dim>(boundary_descriptor);
...
Operator<dim, Number>::update_after_grid_motion()
{
  ...
  if(param.diffusive_problem())
  {
    if(param.use_combined_operator)
      combined_operator.update_after_grid_motion();
    else
      diffusive_operator.update_after_grid_motion();
  }
}
