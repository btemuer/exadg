...
#ifndef INCLUDE_EXADG_DARCY_SOLVER_H_
#define INCLUDE_EXADG_DARCY_SOLVER_H_
...
// application
#include <exadg/darcy/user_interface/declare_get_application.h>
...
create_input_file(std::string const & input_file)
{
  ...
  Darcy::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)->add_parameters(prm);
...
run(std::string const & input_file, MPI_Comm const & mpi_comm)
{
  ...
  std::shared_ptr<Darcy::ApplicationBase<dim, Number>> application =
    Darcy::get_application<dim, Number>(input_file, mpi_comm);

  auto driver = std::make_shared<Darcy::Driver<dim, Number>>(mpi_comm, application);
...
main(int argc, char ** argv)
{
  ...
  if(general.dim == 2 && general.precision == "float")
    ExaDG::run<2, float>(input_file, mpi_comm);
  else if(general.dim == 2 && general.precision == "double")
    ExaDG::run<2, double>(input_file, mpi_comm);
  else if(general.dim == 3 && general.precision == "float")
    ExaDG::run<3, float>(input_file, mpi_comm);
  else if(general.dim == 3 && general.precision == "double")
    ExaDG::run<3, double>(input_file, mpi_comm);
  else
    AssertThrow(false,
...
#endif /* INCLUDE_EXADG_DARCY_SOLVER_H_ */